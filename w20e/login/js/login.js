...
$(function() {
    ...
    var LOGINPAGENAME = R.G.LOGINPAGENAME,
        ...
        var loginAlready = window.location.hash.substr(1);
        if(loginAlready === "adminLoginAlready") {
            $DOM.message.html(MSG.adminLoginAlready);
        }
    ...
    getLoginData("/w20e/goform/getLoginType", function(res) {
        ...
        if(option === 'login') {
            $("#passwordNotice").html('<div>' + '&gt;' + _('登录帐号：') + noticeAcount + '</div><div style="padding-left:10px;">' + _('登录密码：') + noticePwd + '</div>');
        }
